...
def main():
    ...
    # ---- B5 ----
    # Wrap scorers to support inj_by_snr[snr][ev_quiet_pos] indexing
    b5_results = block_b5(score_fns, preprocessed, inj_by_snr, quiet_idx, rfi_idx,
    ...
    for name, r in b5_results.items():
        ...
        vs_ref = f"({row10[1] - 0.927:+.3f} vs mean_pool ref)" if row10 else ""
        print(f"    {name:<22s}: AUC@SNR10={auc10}  floor={tag}  {vs_ref}")
    print(f"    {'mean_pool (ref)':<22s}: AUC@SNR10=0.927  floor=? (encode_separation_test B5)")
    print(f"    {'peak_snr stat (ref)':<22s}: AUC@SNR10=0.813  floor=SNR≈15")
    ...
    if b5_results:
        fig, axes = plt.subplots(1, 2, figsize=(13, 4.5))
        for name, r in b5_results.items():
            axes[0].plot(snrs, [row[1] for row in r["rows"]], marker="o", label=name)
            axes[1].plot(snrs, [row[2] for row in r["rows"]], marker="o", label=name)
        axes[0].axhline(0.927, ls="--", color="red", alpha=0.6, label="mean_pool ref (0.927)")
        axes[0].axhline(0.813, ls=":", color="gray", alpha=0.6, label="peak_snr stat (0.813)")
        ...
        axes[1].set(xlabel="Injection SNR", ylabel="TPR@10% RFI-FP (%)",
                    title="B5: operational TPR vs SNR", ylim=(-5, 105))
